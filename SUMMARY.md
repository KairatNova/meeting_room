...
## Технологии и для чего они нужны

| Технология | Где используется | Для чего в проекте |
|-----------|------------------|--------------------|
| **React + TypeScript** | Frontend | Интерфейс пользователя, типобезопасная разработка страниц и компонентов |
| **Vite** | Frontend | Быстрая сборка и запуск проекта в разработке и production |
| **Tailwind CSS** | Frontend | Быстрая адаптивная вёрстка без отдельного большого CSS |
| **FastAPI** | Backend | REST API, валидация запросов/ответов, Swagger-документация |
| **PostgreSQL** | Backend/DB | Основное хранение данных пользователей, комнат и бронирований |
| **SQLAlchemy** | Backend | Работа с БД через модели и запросы на Python |
| **Alembic** | Backend/DB | Миграции схемы БД (изменения таблиц по версиям) |
| **JWT** | Backend/Auth | Безопасная авторизация и защита приватных маршрутов |
| **Telegram Bot API** | Backend/Auth | Подтверждение аккаунта и отправка кодов пользователю |
| **FullCalendar** | Frontend | Наглядный календарь занятости комнаты по времени |
| **Vitest + Testing Library** | Frontend tests | Проверка UI/клиентской логики и регрессий |
| **Pytest** | Backend tests | Тесты API и бизнес-логики конфликтов бронирования |
